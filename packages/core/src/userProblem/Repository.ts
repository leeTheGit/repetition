...
> {
        ...
        if (params?.withSubmissions && params.userId) {
            ...
            for (let res of result.rows) {
                const items = await submissionRepository.fetchAll({
                    ...
                    limit: params.submissionCount || 5
                })
    ...
    mapToEntity(item: TableType): ModelEntity | ModelError {
        ...
        if ('categorySlug' in item && typeof item.categorySlug !== 'undefined' && item.categorySlug) {
            Entity.categorySlug = item.categorySlug.toString()
        }
