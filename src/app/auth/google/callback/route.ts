...
import { CreateUserFromGoogle } from '@/core/user/actions/create-user-from-google'
import { signupLucia } from "@/lib/auth/auth";
...
export async function GET(request: Request): Promise<Response> {
    ...
    try {
        ...
        // If signing up for the first time on the apex domain we want to 
        // check if the email already exists and error if so
        if (authIntent === 'signup') {
            ...
            const newUser = await CreateUserFromGoogle("google", googleUser )
            if (isError(newUser)) {
                return new Response(null, {
                    status: 500,
                    headers: {
        ...
        // If we're on a subdomain and the user exists we continue with sign in flow
        if (authIntent === 'signin') {
            const existingAccount = await accountRepository.fetchByGoogleId(googleUser.sub)
            ...
            const data = new TextEncoder().encode(googleUser.sub + existingAccount.userId + APP_KEY);
            ...
            return new Response(null, {
                status: 302,
                headers: {
                    Location: `http://${domain}${PLATFORM_DOMAIN}/api/auth/session?type=google&oauthid=${googleUser.sub}&user=${existingAccount.userId}&hash=${hexHash}`
                }
            });
        }
